...
from runtime_fixture_support import JOB_ID, RUN_ID, OTHER_JOB_ID
...
def test_resource_lock_blocks_same_resource_until_released(tmp_path: Path):
    first = ResourceLockSet(tmp_path, JOB_ID, run_id=RUN_ID, heartbeat_seconds=3600)
    second = ResourceLockSet(tmp_path, OTHER_JOB_ID, run_id=RUN_ID, heartbeat_seconds=3600)
    ...
    assert ok is False
    assert "resource locked by " + JOB_ID in reason
...
def test_resource_lock_recovers_corrupt_stale_lock_file(tmp_path: Path):
    lock_set = ResourceLockSet(tmp_path, JOB_ID, run_id=RUN_ID, grace_seconds=0, heartbeat_seconds=3600)
    ...
    assert ok is True
    assert reason == ""
    assert JOB_ID in lock_path.read_text(encoding="utf-8")
    lock_set.release()
...
def test_docker_control_resource_is_based_on_runtime_control_not_only_features():
    resources = _build_resources(
        ...
        {"job_id": JOB_ID, "docker_control": {"mode": "all"}, "features": {"docker": False}},
    )

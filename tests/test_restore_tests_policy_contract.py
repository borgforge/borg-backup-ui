...
def _make_job(root: Path, key: str = job_id('flash_local')) -> None:
    ...
    (jobs_dir / f"{data['job_key']}.json").write_text(json.dumps(data, indent=2), encoding="utf-8")
...
def test_policy_contract_rejects_bad_level(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="level"):
        update_restore_test_policy(cfg, job_id('flash_local'), {"mode": "scheduled", "interval_days": 7, "level": 9})


def test_policy_contract_accepts_valid_payload(tmp_path: Path):
    _make_job(tmp_path)
    cfg = {"BACKUP_SCRIPTS_DIR": str(tmp_path), "RESTORE_TEST_INTERVAL_DAYS": "30"}
    out = update_restore_test_policy(cfg, job_id('flash_local'), {"mode": "manual_only", "interval_days": 7, "level": 1})
...
def test_scheduled_plan_keeps_failed_manual_result_due(tmp_path: Path):
    ...
    (restore_dir / (job_id('flash_local') + ".test")).write_text(json.dumps({
    ...
    assert row["job_key"] == job_id('flash_local')

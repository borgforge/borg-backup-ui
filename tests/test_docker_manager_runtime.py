...
def test_stop_except_selected_keeps_selected_running_containers(monkeypatch):
    running = {"id-paperless", "id-plex", "id-adguard"}
    stop_calls = []
    ...
    monkeypatch.setattr(
        docker_manager,
        "_get_running_name_id_map",
        lambda: {"paperless-ngx": "id-paperless", "plex": "id-plex", "adguard": "id-adguard"},
    )
    monkeypatch.setattr(docker_manager, "_get_running_ids", lambda: sorted(running))
    monkeypatch.setattr(
        docker_manager,
        "_get_container_name",
        lambda cid: {
            "id-paperless": "paperless-ngx",
            "id-plex": "plex",
            "id-adguard": "adguard",
        }[cid],
    )

    result = docker_manager.DockerManager(docker_manager.DockerConfig(stop_wait=0)).stop_except_selected(
        ["plex"],
        "/tmp/backup.log",
    )

    assert result.success is True
    assert result.container_ids == ["id-paperless", "id-adguard"]
    assert result.container_names == ["paperless-ngx", "adguard"]
    assert stop_calls == [["docker", "stop", "-t", "30", "id-paperless", "id-adguard"]]
    assert running == {"id-plex"}
